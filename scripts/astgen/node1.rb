  ...
  class Node
        ...
        each do |key, val|
          @d.pos("syntax #{key.inspect}") do
            val.each do |e|
              ...
              @d.error("#{names.length} unset #{names.length == 1 ? "parameter" : "parameters"} in syntax: #{names.map{|e| e.inspect }.join(", ")}") unless names.length == 0
            end
          end
        end
      end
    ...
    class Prec
      ...
      def chain(*args, **kwargs)
        @items.last.syntax(*args, **kwargs) unless kwargs.length == 0 && args == [nil]
      end

      def resolve(node)
        raise @d.fatal_r("unresolved precedence symbol #{@deferred_item.name.inspect}") unless @deferred_item.nil? || @deferred_item.resolved?
